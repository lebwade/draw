...
class PaymentController extends PublicController {
	...
	public function buy_cart(){
		...
		foreach($id as $k => $v){
		    ...
		    //获取运费
		    $yunfei = true;//$post->where('pid='.intval($pro[$k]['shop_id']))->find();
		    //dump($yunfei);
		    if($pro[$k]['buff']!=''){
	    ...
	    foreach($id as $ks => $vs){
			...
			$weight+=$pro1[$ks]['weight']*$pro1[$ks]['num'];
		}

		//如果需要运费
		if ($yunfei) {
			$yunfei =$this->calcYunfei($weight,$uid);
		...
		//生成订单
		  try {
			...
			foreach($cart_id as $ke => $vl){
				$shop[$ke]=$shopping->where(''.$qz.'shopping_char.uid='.intval($uid).' and '.$qz.'shopping_char.id='.$vl)->join('LEFT JOIN __PRODUCT__ ON __PRODUCT__.id=__SHOPPING_CHAR__.pid')->field(''.$qz.'shopping_char.pid,'.$qz.'shopping_char.num,'.$qz.'shopping_char.shop_id,'.$qz.'shopping_char.buff,'.$qz.'shopping_char.price,'.$qz.'product.price_yh,'.$qz.'product.weight')->find();
				$num+=$shop[$ke]['num'];
                if($shop[$ke]['buff']!=''){
			    ...
			    $weight+=$shop[$ke]['weight']*$shop[$ke]['num'];
			}
			//下单生成运费
			$yunfei =$this->calcYunfei($weight,$uid);
			//if ($_POST['yunfei']) {
			...
			$data['uid']=intval($uid);
			//file_put_contents('./Data/yunfei.txt', $yunfei.PHP_EOL,FILE_APPEND);
            if(!empty($yunfei)){
                $data['yunfei'] = floatval($yunfei);
                $data['price']=floatval($ozprice)+$yunfei;
			}else{
                $data['yunfei'] = 0;
			...
			$data['order_sn']=$this->build_order_no();//生成唯一订单号
			$result = $order->add($data);
			//file_put_contents('./Data/yunfei.txt', $order->getLastSql().PHP_EOL,FILE_APPEND);
		    if($result){
	...
	*/
	...
	/**
	 * [calcYunfei description]
	 * @param  [type] $weight  [description]  重量 所有商品的总重量
	 * @param  [type] $user_id [description]   用户身份id
	 * @return [type]          [description]  
	 */
	private function calcYunfei($weight,$user_id){
		$yunfei =0;
		$addressModel = M('address');
		$map['is_default']=1;
		$map['uid']=intval($user_id);
		$adds_default=$addressModel->where($map)->order('is_default desc,id desc')->find();
		if(is_array($adds_default)&& $adds_default){
			$province_id =$adds_default['sheng'];
		}

		$provinceTable = M('yunfei_price');
		$info = $provinceTable->where(array('province_id'=>$province_id))->find();
		if(is_array($info) && $info){
			$firstprice =$info['firstprice'];
			$continueprice =$info['continueprice'];
			$total_weight =$weight/100; //eg. 5.6kg

			if($total_weight<=1){
				$yunfei=$firstprice;//不足1kg 按1kg
			}else{
				$yunfei =$firstprice+(ceil($total_weight)-1)*$continueprice;
			}


			$yunfei =sprintf('%.2f',$yunfei);
		}
		return $yunfei;
	}
}