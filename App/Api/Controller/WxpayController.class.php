...
class WxpayController extends Controller{
	...
	public function wxpay(){
		...
		$input->SetBody("中防网商城_".trim($order_info['order_sn']));
		$input->SetAttach("中防网商城_".trim($order_info['order_sn']));
		$input->SetOut_trade_no($pay_sn);
		$input->SetTotal_fee(floatval($order_info['amount'])*100);
		//$input->SetTotal_fee(1);
		$input->SetTime_start(date("YmdHis"));
		$input->SetTime_expire(date("YmdHis", time() + 3600));
		$input->SetGoods_tag("中防网商城_".trim($order_info['order_sn']));
		$input->SetNotify_url('https://sc.safetysino.com/Api/Wxpay/notify');
		$input->SetTrade_type("JSAPI");
		$input->SetOpenid($openId);
		$order = \WxPayApi::unifiedOrder($input);
		//file_put_contents('./Data/pay.txt', json_encode($order).PHP_EOL,FILE_APPEND);
