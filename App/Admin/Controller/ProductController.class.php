...
class ProductController extends PublicController{
	...
	*/
	...
	public function save_guide(){
		...
		$cur_pro_buff =$check_info['pro_buff'];//现有属性
		if($_POST['attribute']) {
			//产品规格
			...
			foreach ($attribute as $k => $v) {
				...
				if ($guige_name) {
					foreach ($guige_name as $key => $val) {
						//已经存在的不添加
						$has_attr = M('guige')->where(array('name'=>$val))->find();
						$data=array();
						$data['pid'] = $pro_id;
						$data['attr_id'] = $v;
						$data['name'] = $val;
						...
						$data['addtime'] = time();
						if(is_array($has_attr) && $has_attr){
							$res = M('guige')->where(array('id'=>$has_attr['id']))->save($data);
						}else{
			...
			if($cur_pro_buff){
				$cur_pro_buff_arr = explode(",", $cur_pro_buff);
				$cur_pro_buff_now = array_merge($cur_pro_buff_arr,$attribute);
				$cur_pro_buff_now_this = array_unique($cur_pro_buff_now);
				sort($cur_pro_buff_now_this);
				$str =implode(',', $cur_pro_buff_now_this);
			}else{
				sort($attribute);
				$str = implode(',', $attribute);
			}
