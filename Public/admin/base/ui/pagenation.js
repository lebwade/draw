 ...
 */
var _pages='';
var _Pagenation = function (page, num, total) {
    var _total = total;     // 总条数
    var _page = page;       // 当前页数 默认第一页
    var _pasue_page = page; // 临界点
    var _curr_page = parseInt(page);
    var _num = num;         // 每页显示几条
    _pages = Math.ceil(total / num);  // 总页数
    var _data = '';
    var str = '<div class="fl pages-box"><div class="fl">';
    ...
    if (page == 1) {
        ...
        str +='<a class="btn-prev"> < </a>'+
            '</div> <div class="pages fl">';
    } else {
        // str += '<a class="page-index" onclick="tableScreen.pageTableList(1)" >首页</a><a class="btn-prev" onclick="tableScreen.pageTableList('+(_curr_page-1)+')">上页</a></div> <div class="pages fl">';
    ...
    if (page == _pages) {
        ...
        str += '</div><div class="fl"><a class="btn-next"> > </a></div></div>';
    } else {
    ...
    str += '<div class="fr"><div class="page_info">共' + _total + '条 </div><div class="Jump fl">到 <input type="text" onkeypress="var code=event.keyCode||event.charCode;return code==8||code==46||(code>=37&&code<=40)||(/[0-9\\u0008\\u0037-\\u0040\\u0046]/.test(String.fromCharCode(code)))" class="page-item" id="input-s" maxlength="5">页 共'+_pages+'页 <input type="button" id="btn-go" class="page-item1" value="确定"></div><div class="page_num fl"><div class="fl lh30">每页</div><div class="select page-item1" ><div class="bt select-num">' + _num + '</div><div class="list hide"><a href="javascript:;">10</a><a href="javascript:;">20</a><a href="javascript:;">50</a><a href="javascript:;">100</a></div></div><div class="fl lh30">条</div></div></div>';
    $('#pageNation').html(str);
    ...
    if(_bidui <= _page_select_layer_h){
        $('#pageNation .select .list').css('top',-parseInt(_page_select_layer_h));
    }

}

$('body').on('click', '#pageNation .select-num', function (e) { // 选择每页显示条数
    var event = e || window.event;
    event.stopPropagation();
    $(this).next('.list').show();
});

$('body').on('click', '#pageNation .list a', function (e) { // 选择每页显示条数
    _this = $(this);
    _this.addClass('on').siblings().removeClass('on');
    _this.closest('.list').hide();
    var search=$('#mem_name').val() || '';
    var _val = _this.text();
    $('#pageNation .select-num').text(_val);
    tableScreen.pageTableListNum(_val);
});

// prev
$('#pageNation').on('click', '.btn-prev', function () { // 选择每页显示条数
    var __page=$('#pageNation').find('.pages a.on').text();
    if(__page>1){
        __page = $('#pageNation').find('.pages a.on').text() - 1;
        tableScreen.pageTableList(__page);
    }
});

// next
$('#pageNation').on('click', '.btn-next', function () { // 选择每页显示条数
    var __page=parseInt($('#pageNation').find('.pages a.on').text());
    if(__page<_pages){
        var __page= parseInt($('#pageNation').find('.pages a.on').text())+1;
        tableScreen.pageTableList(__page);
    }
});

//go
$('#pageNation').on('click', '#btn-go', function () {
    var index=parseInt($("#input-s").val());
    if(index>0){
        tableScreen.pageTableList(index);
    }
    return false;
})
